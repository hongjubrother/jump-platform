...
class Game:
    ...
    def load_data(self):
        ...
        self.boost_sound = pg.mixer.Sound(path.join(self.snd_dir, 'powerup.mp3')) 
    ...
    def new(self):
        ...
        for plat in PLATFORM_LIST:
            Platform(self, *plat)
        self.mob_timer = 0
        pg.mixer.music.load(path.join(self.snd_dir, 'Kirby_bgm1.mp3'))
        self.run()
            ...
            if hits:
                ...
                for hit in hits:
                    if hit.rect.bottom > lowest.rect.bottom:
                        lowest = hit
                # 커비 이미지 파일에 따라 새로 고쳐야됨(발 platform 에 닫는 코드)        
    ...
    def show_start_screen(self):
        ...
        pg.mixer.music.load(path.join(self.snd_dir, 'Kirby_opening_bgm.mp3'))
        pg.mixer.music.play(loops=-1)
        self.screen.fill(BGCOLOR)
    ...
    def show_go_screen(self):
        ...
        if not self.running:
            return
        pg.mixer.music.load(path.join(self.snd_dir, 'Kirby_bgm3.mp3'))
        self.screen.fill(BGCOLOR)
